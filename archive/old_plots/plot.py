...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Plot PRB/FastPRB Results")
...
def plot_dataset_group(dataset_name, experiments):
    ...
    for idx, exp in enumerate(experiments):
        ...
        plt.plot(rounds, mean_regret, label=info['label'], color=line_color, 
                 linewidth=LINE_WIDTH, linestyle=current_ls, zorder=current_zorder)

    # 纵轴按曲线真实最大 regret 自动设置；横轴 round 为 0..T-1
    max_regret_plot = max(float(np.max(exp['data'][:, 1])) for exp in experiments)
    plt.ylim(0, max_regret_plot)

    plt.title(std_name, fontsize=TITLE_SIZE, fontweight='bold', pad=20)
    plt.xlabel("Rounds", fontsize=LABEL_SIZE, fontweight='bold')
    plt.ylabel("Regret", fontsize=LABEL_SIZE, fontweight='bold')
    ...
    plt.savefig(out_path_rounds_png, dpi=300)
    plt.savefig(out_path_rounds_pdf)
    print(f"   -> Saved Regret Plot: {out_path_rounds_png}")
    plt.close()

    # =========================================================
    # 图二：Accuracy vs Time (分进程独立图) -> 存放在子目录
    # =========================================================
    ...
    for exp in experiments:
        ...
        for run_id in range(num_runs):
            ...
            plt.savefig(os.path.join(sub_dir_path, out_name), dpi=100) # 这里dpi不用太高，主要是为了预览挑选
            plt.close()
