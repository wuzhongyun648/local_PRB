...
log_dir="$repo_dir/log/formal_local_prb2_${run_stamp}"
...
readonly -a datasets=(Collab PPA)
readonly -a rounds=(5000 5000)
readonly -a lr1s=(0.01 0.01)
readonly -a lr2s=(0.004 0.004)
readonly -a gpus=(0 1)
...
validate_result() {
  local task_log="$1" dataset="$2" result_dir
  ...
  "$python_bin" "$repo_dir/scripts/validate_a100_result.py" \
    ...
    --rounds 5000 --runs "$runs" --evaluation-every "$evaluation_every"
}
...
probe_dir="$result_root/.local_prb2_write_test_${run_stamp}"
...
[[ "$commit" == "14438c444067f678b5eb283073d258d070c19aac" ]] || {
...
for index in "${!datasets[@]}"; do
  ...
  nohup "$python_bin" -u main.py \
    --graph_name "$dataset" --method PRB --power_T 50 --T "${rounds[$index]}" \
    --lr1 "${lr1s[$index]}" --lr2 "${lr2s[$index]}" --alpha 0.85 \
    --runs "$runs" --workers "$workers" --seed 0 --n_neg 9 \
    --init_hops 0 --init_topk 0 --evaluation_every "$evaluation_every" --hidden 100 \
    > "$task_log" 2>&1 < /dev/null &
  pid=$!
  pids+=("$pid")
  task_logs+=("$task_log")
  log_master "TASK_START method=PRB dataset=$dataset gpu=${gpus[$index]} pid=$pid rounds=${rounds[$index]} log=$task_log"
done

failed=0
declare -a done_flags=(0 0)
remaining=2
while ((remaining > 0)); do
  for index in "${!pids[@]}"; do
    ...
    if ! kill -0 "$pid" 2>/dev/null; then
      ...
      if wait "$pid" && validate_result "$task_log" "$dataset" >> "$master_log" 2>&1; then
...
if ((failed != 0)); then
  log_master "LOCAL_PRB2_FAILED"
  exit 1
fi
log_master "LOCAL_PRB2_VALIDATED"
